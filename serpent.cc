...
int makeKey(keyInstance *key, uint8_t direction, int keyLen, char *keyMaterial){
  ...
  memcpy(key->keyMaterial, keyMaterial, MAX_KEY_SIZE+1);
  memcpy(key->key, keyMaterial, MAX_KEY_SIZE);
...
int blockEncrypt(cipherInstance *cipher, keyInstance *key, uint8_t *input, int inputLen, uint8_t *outBuffer){
  ...
  switch(cipher->mode)
	...
	serpent_encrypt((uint32_t*)input, (uint32_t*)outBuffer, key->subkeys);
      return inputLen;

    case MODE_CBC:
      t[0] = ((uint32_t*)cipher->IV)[0];
      t[1] = ((uint32_t*)cipher->IV)[1];
      t[2] = ((uint32_t*)cipher->IV)[2];
      t[3] = ((uint32_t*)cipher->IV)[3];
      for(b=0; b<inputLen; b+=128, input+=16, outBuffer+=16)
	{
	  t[0] ^= ((uint32_t*)input)[0];
	  t[1] ^= ((uint32_t*)input)[1];
	  t[2] ^= ((uint32_t*)input)[2];
	  t[3] ^= ((uint32_t*)input)[3];
	  serpent_encrypt(t, t, key->subkeys);
	  ((uint32_t*)outBuffer)[0] = t[0];
	  ((uint32_t*)outBuffer)[1] = t[1];
	  ((uint32_t*)outBuffer)[2] = t[2];
	  ((uint32_t*)outBuffer)[3] = t[3];
	}
      ((uint32_t*)cipher->IV)[0] = t[0];
      ((uint32_t*)cipher->IV)[1] = t[1];
      ((uint32_t*)cipher->IV)[2] = t[2];
      ((uint32_t*)cipher->IV)[3] = t[3];
      return inputLen;
...
int blockDecrypt(cipherInstance *cipher, keyInstance *key, uint8_t *input, int inputLen, uint8_t *outBuffer){
  ...
  switch(cipher->mode)
    ...
    case MODE_ECB:
      for(b=0; b<inputLen; b+=128, input+=16, outBuffer+=16)
	serpent_decrypt((uint32_t*)input, (uint32_t*)outBuffer, key->subkeys);
      return inputLen;
...
void serpent_encrypt(uint32_t plaintext[4], uint32_t ciphertext[4], uint32_t subkeys[33][4]){
  ...
  static uint32_t x0, x1, x2, x3;
  static uint32_t y0, y1, y2, y3;
...
static keyInstance keyI; 
static cipherInstance cipherI; 
int i=0, rc=0;
    
    
  if (ciphertext == NULL && plaintext == NULL && key == NULL && len == 0){
    memset(&keyI, 0, sizeof(keyInstance));
    memset(&cipherI, 0, sizeof(cipherInstance)); 
    return 1;
  }
  ...
  if ( (rc=cipherInit(&cipherI, MODE_ECB, "")) <= 0 )
    return -4;

  
   if ( (rc=makeKey(&keyI, DIR_ENCRYPT, 256, (char *)key)) <= 0 )
    return -5;

  for (i=0; i<len; i+=16){
      if ( (rc=blockEncrypt(&cipherI, &keyI, (unsigned char *)plaintext+i, 128, (unsigned char *)ciphertext+i)) <= 0)
            return -6;
  }
    
  return 0;
    
} /* End of serpent_encrypt_buffer() */
...
int i=0, rc=0;
  ...
  for (i=0; i<len; i+=16){
      if ( (rc=blockDecrypt(&cipherI, &keyI, (unsigned char *)ciphertext+i, 128, (unsigned char *)plaintext+i)) <= 0)
            return -6;
  }
    
  return 0;
    
} /* End of serpent_decrypt_buffer() */
