...
void Blowfish_Init(BLOWFISH_CTX *ctx, unsigned char *key, int keyLen) {
  int i, j, k;
  static uint32_t data, datal, datar;
  ...
  for (i = 0; i < N + 2; i += 2) {
    Blowfish_Encrypt(ctx, &datal, &datar, &datal, &datar);
    ctx->P[i] = datal;
    ctx->P[i + 1] = datar;
  }
...
/** Encrypts the supplied "plaintext" buffer using "key" as the encryption key.
  * The result is stored in buffer "ciphertext". Argument "len" indicates the 
  * length of the input buffer. Output buffer must be able to hold at least 
  * "len" bytes. If key is NULL this function assumes that Blowfish_Init was 
  * already called and that the BLOWFISH_CTX var has been initialized. This 
  * should save some CPU when encrypting several times with the same key. If 
  * "ciphertext", "plaintext" and "key" are NULL, var BLOWFISH_CTX is zeroed.
  * WARNING: It is assumed that buffer "key" is 256 bits long.                */
int blowfish_encrypt_buffer(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key, int len){

   
 static BLOWFISH_CTX ctx; /* Required by Paul Kocher's implementation */
 int i=0;
 ...
 if(len %8 != 0 || len == 0 ) /* Buffer must equal an exact number of 64bit blocks */
    return -2;
 
 /* BLOWFISH_CTX initialization using the supplied encryption key             */    
 if (key != NULL) /* If called with NULL, no need to initialize this time     */
    Blowfish_Init(&ctx, key, 32);  

 
 /* Encrypt block by block. Blocks start at the following offsets             */
 /* 0 8 16 24 32 40 48 56 64 72...                                            */
 for (i=0; i < len ; i+=8  ){   
    /* Blowfish_Encrypt(BLOWFISH_CTX variable,first 4 bytes, last 4 bytes)    */
    Blowfish_Encrypt(&ctx, (uint32_t *)(plaintext+i), (uint32_t *)(plaintext+i+4), (uint32_t *)(ciphertext+i), (uint32_t *)(ciphertext+i+4)); 
 }   
  
 return 0;
    
} /* End of blowfish_encrypt_buffer() */
...
/** Decrypts the supplied "ciphertext" buffer using "key" as the decryption key.
  * The result is stored in buffer "plaintext". Argument "len" indicates the 
  * length of the input buffer. Output buffer must be able to hold at least 
  * "len" bytes. If key is NULL this function assumes that Blowfish_Init was 
  * already called and that the BLOWFISH_CTX var has been initialized. This 
  * should save some CPU when decrypting several times with the same key. If 
  * "ciphertext", "plaintext" and "key" are NULL, var BLOWFISH_CTX is zeroed.
  * WARNING: It is assumed that buffer "key" is 256 bits long.                */
int blowfish_decrypt_buffer(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key, int len){
 ...
 for (i=0; i < len ; i+=8  ){   
    /* Blowfish_Encrypt(BLOWFISH_CTX variable,first 4 bytes, last 4 bytes)    */
    Blowfish_Decrypt(&ctx, (uint32_t *)(plaintext+i), (uint32_t *)(plaintext+i+4), (uint32_t *)(ciphertext+i), (uint32_t *)(ciphertext+i+4)); 
 }   
  
 return 0;
    
} /* End of blowfish_decrypt_buffer() */
