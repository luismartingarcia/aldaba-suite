...
/** Returns a decent random long int suitable to seed a PRNG. It reads random
  * bytes from device /dev/urandom and XORs the result with the supplied seed.
  * Pass a seed value of 0 if you trust system's entropy pool. The function
  * returns a nice random long integer on success and OP_FAILURE  in case of
  * error.    */

/* TODO: "upgrade" user supplied seed to type long int. */
long int get_decent_random_seed(int seed){
 int devrandom_fd=0;
 long int new_seed=0;

  /* Open system's random number generator /dev/urandom  */
  devrandom_fd=open("/dev/random", O_RDONLY | O_NONBLOCK);

  /* If device was opened correctly, read some bytes. */
  if (devrandom_fd != -1 ){
      if ( read(devrandom_fd, &new_seed, sizeof(long int)) != sizeof(long int) ){
            close(devrandom_fd);
            return OP_FAILURE;
      }
      else{
          close(devrandom_fd);
          return new_seed ^ seed;
      }
  }
  else
    return OP_FAILURE;
} /* End of get_decent_random_seed() */
...
} /* End of fill_buffer_with_random_data() */
...
static int set_socket_blocking_state(int fd, bool blocking){
  int flags=-1;
...
int setSocketNonBlocking(int fd){
  return set_socket_blocking_state(fd, true);
...
static int parse_unsigned_number(const char *str, unsigned int bits, void *dstbuff){
  ...
  if(!strcasecmp(str, "rand") || !strcasecmp(str, "random")){
    fill_buffer_with_random_data((char *)dstbuff, bits/8);
    return OP_SUCCESS;
  }
