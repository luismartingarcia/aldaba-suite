...
/* The original rijndael code used the definition u32. It's better to link */
...
/** Expand the cipher key into the encryption key schedule. @return the number
  * of rounds for the given cipher key size.                                  */
...
/** Expand the cipher key into the decryption key schedule. @return the number
  * of rounds for the given cipher key size.                                  */
...
{
  static u32 s0, s1, s2, s3, t0, t1, t2, t3;
  #ifndef FULL_UNROLL
    static int r;
  #endif /* ?FULL_UNROLL */
  /*
...
/** Encrypts the first 16 bytes of buffer "plaintext" using the supplied key.
  * The result is stored in buffer "ciphertext". WARNING: Assuming key is 256
  * bits!                                                                     */
int rijndael_encrypt16(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key){

  static uint32_t rk[RKLENGTH(KEYBITS)];
  int nrounds;
...
/** Decrypts the first 16 bytes of buffer "ciphertext" using the supplied key.
  * The result is stored in buffer "plaintext". WARNING: Assuming key is 256
  * bits!                                                                     */
...
/** Encrypts the first 32 bytes of buffer "plaintext" using the supplied key.
  * The result is stored in buffer "ciphertext". WARNING: Assuming key is 256
  * bits!                                                                     */
int rijndael_encrypt32(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key){
  ...
  nrounds = rijndaelSetupEncrypt(rk, key, 256);
  rijndaelEncrypt(rk, nrounds, plaintext, ciphertext);
  rijndaelEncrypt(rk, nrounds, plaintext+16, ciphertext+16);
  return 0;

} /* End of rijndael_encrypt32() */
...
/** Decrypts the first 32 bytes of buffer "ciphertext" using the supplied key.
  * The result is stored in buffer "plaintext". WARNING: Assuming key is 256
  * bits!                                                                     */
int rijndael_decrypt32(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key){
  ...
  nrounds = rijndaelSetupDecrypt(rk, key, 256);
  rijndaelDecrypt(rk, nrounds, ciphertext, plaintext);
  rijndaelDecrypt(rk, nrounds, ciphertext+16, plaintext+16);

  return 0;

} /* End of rijndael_decrypt32() */
...
/** Encrypts the first "len" bytes of buffer "plaintext" using the supplied key.
...
int rijndael_encrypt_buffer(unsigned char *ciphertext, unsigned char *plaintext, unsigned char *key, int len){
  ...
  if (ciphertext == NULL && plaintext == NULL && key == NULL && len == 0){
    memset(rk, 0, sizeof(rk)); /* This size should be 240 ( [(256/8)+28] * sizeof(uint32_t) =  60 * 4 = 240 )*/
    return 1;
  }
  else if(len %16 != 0 || len == 0 )
    return -1;
...
} /* End of rijndael_encrypt_buffer() */
...
/** Decrypts the first "len" bytes of buffer "ciphertext" using the supplied
...
} /* End of rijndael_decrypt_buffer() */
