...
PKDataStrong6::PKDataStrong6(){
    this->reset();
} /* End of PKDataStrong6 contructor */


PKDataStrong6::~PKDataStrong6(){

} /* End of PKDataStrong6 destructor */


/** Puts the object back to its initial state */
void PKDataStrong6::reset(){
...
u8 *PKDataStrong6::getBufferPointer(){
  return (u8 *)(&h);
...
u8 *PKDataStrong6::getBufferPointer(u32 *len){
  if(len!=NULL)
    *len=(u32)this->length;
  return (u8 *)(&h);
} /* End of getBufferPointer() */
...
/** Stores supplied data in the internal buffer so the information
  ...
  * @warning  The PKDataStrong6 class is able to hold a maximum of
  * PK_STRONG_IPv6_DATA_LEN bytes. If the supplied buffer is longer than that, it
  * will fatal().
  * @warning Supplied len MUST be PK_STRONG_IPv6_DATA_LEN bytes (min IP header
  * length).
  * @return OP_SUCCESS on success. Fatals on error. */
int PKDataStrong6::storeRecvData(const u8 *buf, size_t len){
  if(buf==NULL || len!=PK_STRONG_IPv6_DATA_LEN){
      fatal(OUT_2, "%s(): Invalid parameters.", __func__);
  }else{
    this->reset(); /* Re-init the object, just in case the caller had used it already */
    this->length=PK_STRONG_IPv6_DATA_LEN;
    memcpy(&(this->h), buf, PK_STRONG_IPv6_DATA_LEN);
  }
 return OP_SUCCESS;
...
void PKDataStrong6::getKnockData(u8 *buff, u32 *final_len){
  if(buff!=NULL)
...
/** Returns a pointer to the knock data.
  ...
  * @param len is a pointer to an unsigned integer var where the knock data
  * length is stored by this function (if not NULL). */
u8 *PKDataStrong6::getKnockData(u32 *len){
  if(len!=NULL)
    *len=(u32)this->length;
  return (u8 *)(&h);
} /* End of getKnockData() */
...
/** Stores caller supplied knock data in the object's internal buffer.
  ...
  * @warning supplied pointer MUST hold at least PK_STRONG_IPv6_DATA_LEN bytes.
...
int PKDataStrong6::setKnockData(u8 *buff){
  if(buff==NULL)
    return OP_FAILURE;
  memcpy(&(this->h), buff, PK_STRONG_IPv6_DATA_LEN);
  this->length=PK_STRONG_IPv6_DATA_LEN;
  return OP_SUCCESS;
} /* End of getKnockData() */
...
int PKDataStrong6::setKnockData(u8 *buffer, u32 *final_len, IPAddress ip, tcp_port_t port, int action, u8 *key, size_t keylen){

  u8 hmac[HMAC_SHA256_LEN];
  pk_strong_header_ipv6 *p = (pk_strong_header_ipv6 *)buffer;

  if(buffer==NULL)
     fatal(OUT_2, "%s(): NULL parameter supplied", __func__);
  if( ip.getVersion()!=AF_INET6 )
     fatal(OUT_2, "%s() Expected IP version 6 address. Please report a bug.", __func__);

  memcpy(p->nonce, o.rand.getRandomData(PK_STRONG_IPv6_NONCE_LEN), PK_STRONG_IPv6_NONCE_LEN);

    /* Set or unset the least significant bit */
  if(action==ACTION_OPEN)
    p->nonce[PK_STRONG_IPv6_NONCE_LEN-1] |= 0x01;
  else
    p->nonce[PK_STRONG_IPv6_NONCE_LEN-1] &= ~(0x01);

  p->timestamp = htonl( (u32)time(NULL) );
  p->knock_ip=ip.getIPv6Address();
  p->knock_port=htons(port);
  memset(p->mac, 0, PK_STRONG_IPv6_MAC_LEN);
  ...
  HMAC_SHA256::hmac_sha256(key, keylen, buffer, PK_STRONG_IPv6_DATA_LEN, hmac, HMAC_SHA256_LEN);

  /* Copy the 6 most significant bytes of the hash */
  memcpy(p->mac, hmac, PK_STRONG_IPv6_MAC_LEN);

  /* Update length */
  if(final_len!=NULL)
    *final_len=PK_STRONG_IPv6_DATA_LEN;
...
/** Encode knock data.
  ...
  * @param port is the knock port.
  * @param action MUST be one of OPEN_PORT or CLOSE_PORT */
int PKDataStrong6::setKnockData(IPAddress ip, tcp_port_t port, int action, u8 *key, size_t keylen){
  this->length=PK_STRONG_IPv6_DATA_LEN;
  return setKnockData((u8 *)&this->h, &this->length, ip, port, action, key, keylen);
}

IPAddress PKDataStrong6::getAddress(){
  IPAddress ip;
...
tcp_port_t PKDataStrong6::getPort(){
  return ntohs(this->h.knock_port);
}

int PKDataStrong6::getAction(){
  if( this->h.nonce[PK_STRONG_IPv6_NONCE_LEN-1]%2==0 )
    return ACTION_CLOSE;
  else
    return ACTION_OPEN;
}/* End of getPort() */

u32 PKDataStrong6::getTimestamp(){
  return ntohl(this->h.timestamp);
} /* End of getTimestamp() */


u8 *PKDataStrong6::getNonce(size_t *final_len){
  if(final_len!=NULL)
    *final_len=PK_STRONG_IPv6_NONCE_LEN;
  return this->h.nonce;
} /* End of getNonce() */


u8 *PKDataStrong6::getNonce(){
  return this->getNonce(NULL);
} /* End of getNonce() */


size_t PKDataStrong6::getNonceLength(){
    return PK_STRONG_IPv6_NONCE_LEN;
} /* End of getNonceLength() */


bool PKDataStrong6::validateKnockData(u8 *key, size_t keylen){
  u8 hmac[HMAC_SHA256_LEN];
  u8 backup[PK_STRONG_IPv6_MAC_LEN];
  memcpy(backup, this->h.mac,PK_STRONG_IPv6_MAC_LEN);
  memset(this->h.mac, 0, PK_STRONG_IPv6_MAC_LEN);
  HMAC_SHA256::hmac_sha256(key, keylen, (u8 *)&this->h, PK_STRONG_IPv6_DATA_LEN, hmac, HMAC_SHA256_LEN);
  memcpy(this->h.mac, backup, PK_STRONG_IPv6_MAC_LEN);
  if(memcmp(backup, hmac, PK_STRONG_IPv6_MAC_LEN)==0)
    return true;
  else
    return false;
...
const char *PKDataStrong6::toString(){
