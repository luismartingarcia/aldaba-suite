...
u8 *PKDataLight6::getBufferPointer(){
  return (u8 *)(&h);
...
u8 *PKDataLight6::getBufferPointer(u32 *len){
  if(len!=NULL)
    *len=(u32)this->length;
  return (u8 *)(&h);
} /* End of getBufferPointer() */
...
/** Stores supplied data in the internal buffer so the information
  ...
  * @warning  The PKDataLight6 class is able to hold a maximum of
  * PK_LIGHT_IPv6_DATA_LEN bytes. If the supplied buffer is longer than that, it
  * will fatal().
  * @warning Supplied len MUST be PK_LIGHT_IPv6_DATA_LEN bytes (min IP header
  * length).
  * @return OP_SUCCESS on success. Fatals on error. */
int PKDataLight6::storeRecvData(const u8 *buf, size_t len){
  if(buf==NULL || len!=PK_LIGHT_IPv6_DATA_LEN){
      fatal(OUT_2, "%s(): Invalid parameters.", __func__);
  }else{
    this->reset(); /* Re-init the object, just in case the caller had used it already */
    this->length=PK_LIGHT_IPv6_DATA_LEN;
    memcpy(&(this->h), buf, PK_LIGHT_IPv6_DATA_LEN);
  }
 return OP_SUCCESS;
...
void PKDataLight6::getKnockData(u8 *buff, u32 *final_len){
  if(buff!=NULL)
...
/** Returns a pointer to the knock data.
  ...
  * @param len is a pointer to an unsigned integer var where the knock data
  * length is stored by this function (if not NULL). */
u8 *PKDataLight6::getKnockData(u32 *len){
  if(len!=NULL)
    *len=(u32)this->length;
  return (u8 *)(&h);
} /* End of getKnockData() */
...
int PKDataLight6::setKnockData(u8 *buff){
  if(buff==NULL)
    return OP_FAILURE;
  memcpy(&(this->h), buff, PK_LIGHT_IPv6_DATA_LEN);
  this->length=PK_LIGHT_IPv6_DATA_LEN;
  return OP_SUCCESS;
} /* End of getKnockData() */
...
int PKDataLight6::setKnockData(u8 *buffer, u32 *final_len, IPAddress ip, tcp_port_t port, int action, u8 *key, size_t keylen){

  u8 hmac[HMAC_SHA256_LEN];
  pk_light_header_ipv6 *p = (pk_light_header_ipv6 *)buffer;
  ...
  memset(p->mac, 0, PK_LIGHT_IPv6_MAC_LEN);

  buffer[PK_LIGHT_IPv6_DATA_LEN-1] = (action==ACTION_OPEN) ? 0x01 : 0x00;
  ...
  HMAC_SHA256::hmac_sha256(key, keylen, buffer, PK_LIGHT_IPv6_DATA_LEN, hmac, HMAC_SHA256_LEN);

  /* Copy the 6 most significant bytes of the hash */
  memcpy(p->mac, hmac, PK_LIGHT_IPv6_MAC_LEN);

  /* Set or unset the least significant bit */
  if(action==ACTION_OPEN)
    p->mac[PK_LIGHT_IPv6_MAC_LEN-1] |= 0x01;
  else
    p->mac[PK_LIGHT_IPv6_MAC_LEN-1] &= ~(0x01);

  /* Update length */
  if(final_len!=NULL)
    *final_len=PK_LIGHT_IPv6_DATA_LEN;
...
/** Encode knock data.
  ...
  * @param port is the knock port.
  * @param action MUST be one of OPEN_PORT or CLOSE_PORT */
int PKDataLight6::setKnockData(IPAddress ip, tcp_port_t port, int action, u8 *key, size_t keylen){
  
  this->length=PK_LIGHT_IPv6_DATA_LEN;
  return setKnockData((u8 *)&this->h, &this->length, ip, port, action, key, keylen);
}

IPAddress PKDataLight6::getAddress(){
  IPAddress ip;
...
tcp_port_t PKDataLight6::getPort(){
  return ntohs(this->h.knock_port);
}

int PKDataLight6::getAction(){
  if( this->h.mac[PK_LIGHT_IPv6_MAC_LEN-1]%2==0 )
    return ACTION_CLOSE;
  else
    return ACTION_OPEN;
}/* End of getPort() */
...
bool PKDataLight6::validateKnockData(u8 *key, size_t keylen){
 IPAddress ip;
 tcp_port_t port;
 int action;
 u8 *aux=(u8*)&this->h;
 u8 buff[PK_LIGHT_IPv6_DATA_LEN];
 memset(buff, 0, PK_LIGHT_IPv6_DATA_LEN);

 /* Reconstruct original parameters */
 ip.setAddress(this->h.knock_ip);
 port=ntohs(this->h.knock_port);
 action=(aux[PK_LIGHT_IPv6_DATA_LEN-1]%2==0) ? ACTION_CLOSE : ACTION_OPEN;
 ...
 if ( memcmp(buff, &this->h, PK_LIGHT_IPv6_DATA_LEN)==0 )
    return true;
 else
    return false;
}
...
const char *PKDataLight6::toString(){
  static char buffer[256];
  snprintf(buffer, 256, "%s;%d;", this->getAddress().toString(), this->getPort());
  for(int i=0; i<PK_LIGHT_IPv6_MAC_LEN; i++)
    snprintf(buffer+strlen(buffer), 256-strlen(buffer), "%02x", this->h.mac[i]);
  return buffer;
